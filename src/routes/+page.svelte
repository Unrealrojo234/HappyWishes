<script>
	...
	const handleUrl = async (e) => {
		...
		try {
			...
			ShortUrlLink = data.data.tiny_url;
			console.log(ShortUrlLink);
			isVisible = false;
			return json({ shortUrl: data.data.tiny_url });
		} catch (err) {
			console.log('Error', err);
			return json({ error: 'Failed to connect to TinyURL API.' }, { status: 500 });
		}
	};
</script>
...
<main class="container-fluid" id="top">
	...
	>
		...
		{/if}
		<div id="content-div">
			<form class="form-control" action="submit" onsubmit={handleUrl}>
				<label for="input">Enter your name:</label>
				<input bind:value={name} type="text" class="form-control" required />
				<br />
				<label for="input">Enter your message: (Optional)</label>
				<textarea bind:value={msg} rows="5" class="form-control"></textarea>
				<br />
				<div class="sp text-center">
					<button class="sparkle-button" type="submit">
		...
		{#if ShortUrlLink}
			<div class="container-fluid text-center">
				<span>Website: </span>
				<a target="_blank" href={ShortUrlLink}>{ShortUrlLink} </a>
				<span style="display: inline-block;margin-left:2rem;"
					><CopyButton link={ShortUrlLink} /></span
				>
				<br /><br />
				<!-- Checking is share api is availabel in browser -->
				{#if navigator.share}
					<span style="display: inline-block;margin-left:2rem;"
						><ShareButton url={ShortUrlLink} /></span
					>
				{/if}
			</div>
		{/if}
